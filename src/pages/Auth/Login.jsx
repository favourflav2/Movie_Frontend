...
export default function Login() {
  ...
  function handleSubmit() {
    ...
    if(inputState.email && inputState.password){
      dispatch(logIn({formData:inputState,toast,navigate}))
    }else{
    ...
    <Box className="mt-[90px] flex w-full h-[830px] sm:h-[920px]">
      ...
      <Box className="flex w-full h-full">
        ...
        <Box className="sm:w-[70%] w-full bg-gray-400/30 flex items-center justify-center">
          {/* Content Container */}
          <Box className="flex flex-col w-[360px] sm:w-[380px] sm:h-[450px] md:w-[400px] lg:w-[500px] xl:w-[600px] rounded-xl border-2 border-indigo-500 bg-white/50 items-center">
            {/* Inner Box */}
            <Box className="flex flex-col p-4  w-[95%] h-full ">
              <Box className="flex flex-col items-center">
                ...
                {/* Buttons */}
                <Box className="flex flex-col items-center  w-full mt-3">
                  <Button
                    variant="contained"
